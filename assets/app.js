...
(function () {
  ...
  /* Journey strip: you-are-here within your track (leader 6, builder 8) */
  var crumb = document.querySelector(".toolbar .crumb");
  var mastheadWrap = document.querySelector(".masthead .wrap");
  if (crumb && mastheadWrap) {
    var LEADER_PAGES = ["a1-strategy-is-deciding.html", "a2-one-number-or-two-kinds.html", "a3-look-at-100-mistakes.html", "a4-where-the-error-comes-from.html", "a5-the-pipeline-hides-its-weakest-part.html", "a6-the-one-page-memo.html"];
    var BUILDER_PAGES = ["p1-dev-and-test-sets.html", "p2-first-system-in-a-week.html", "p3-error-analysis-spreadsheet.html", "p4-eyeball-and-blackbox.html", "p5-bias-and-variance.html", "p6-learning-curves.html", "p7-different-distributions.html", "p8-optimization-verification.html", "p9-error-analysis-by-parts.html", "p10-what-to-fix-next-bench.html"];
    var ml = crumb.textContent.match(/Leader session (\d+) of 6/);
    var mb = crumb.textContent.match(/Practitioner session (\d+) of 10/);
    var m = ml || mb;
    if (m) {
      var current = parseInt(m[1], 10);
      var pages = ml ? LEADER_PAGES : BUILDER_PAGES;
      var letter = ml ? "Leader session " : "Practitioner session ";
      var journey = document.createElement("div");
      journey.className = "journey";
      var jl = document.createElement("span");
      jl.className = "jlabel";
      jl.textContent = ml ? "Leader track" : "Practitioner track";
...
(function () {
  "use strict";
  var PASSPORT_KEY = "lwp-passport:ml-strategy";
  var TOTAL_SESSIONS = 16;
  var SESSION_KEYS = ["a1", "a2", "a3", "a4", "a5", "a6", "p1", "p2", "p3", "p4", "p5", "p6", "p7", "p8", "p9", "p10"];
  var SESSION_LABELS = ["L1", "L2", "L3", "L4", "L5", "L6", "P1", "P2", "P3", "P4", "P5", "P6", "P7", "P8", "P9", "P10"];
  ...
  var FNS = {
    ...
    contrast: function (s) {
      ...
      if (hx.length < 2) return { error: "Give me two hex colours, for example  #FFFFFF #0E7490" };
      function lum(h) {
        h = h.replace("#", "");
  ...
  var cmA = crumbEl && crumbEl.textContent.match(/Leader session (\d+) of 6/);
  var cmB = crumbEl && crumbEl.textContent.match(/Practitioner session (\d+) of 10/);
  var here = cmA ? "a" + cmA[1] : (cmB ? "p" + cmB[1] : null);
  var hereLabel = cmA ? "Leader session " + cmA[1] : (cmB ? "Practitioner session " + cmB[1] : "");
  ...
  function ppPaintPill() {
    ...
    pill.title = n === 0 ? "Answer all three quiz questions in a session to stamp it."
      : "Stamped: " + SESSION_KEYS.filter(function (k) { return got[k]; }).join(", ");
      ...
      var watch = setInterval(function () {
        ...
        if (done >= qs.length) {
          ...
          if (!got[here]) {
            ...
            var score = document.querySelector(".quiz-score");
            if (score) {
              score.textContent += "  🎫 " + hereLabel + " stamped in your passport (" +
  ...
  if (strip) {
    ...
    SESSION_KEYS.forEach(function (key, i) {
      var card = cards[i];
      var href = card ? (card.getAttribute("href") ||
        ((card.querySelector("a") || {}).getAttribute ? card.querySelector("a").getAttribute("href") : null)) : null;
      var a = el("a", "pp-stamp" + (got2[key] ? " got" : ""));
      a.href = href || "#";
      a.appendChild(el("i", null, got2[key] ? "🎫" : "○"));
      a.appendChild(el("b", null, SESSION_LABELS[i]));
      a.title = (got2[key] ? "Stamped: " : "Not stamped yet: ") +
        (key.charAt(0) === "a" ? "leader session " : "practitioner session ") + key.slice(1);
      strip.appendChild(a);
    });
    var note = el("p", "pp-note", ppCount(got2) === 0
      ...
      : ppCount(got2) >= TOTAL_SESSIONS
        ? TOTAL_SESSIONS + " of " + TOTAL_SESSIONS + ". Both tracks finished - the bench's top rung is now open."
